---
title: "Year to Year IS210 Bid Analysis"
output:
...
## Plot the data 1 -> Bidding Price vs Instructor , Section
```{r Bidding Price vs Instructor and Section, echo=FALSE}
...
ggplot(bidding_data_filtered) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size=5)) + geom_hline(data = averages, aes(yintercept = avg_median_bid, color = "Avg Median     Bid"), linetype = "dashed") +
  geom_hline(data = averages, aes(yintercept = avg_min_bid, color = "Avg Min Bid"), linetype = "dotted") +
  geom_text(data = averages, aes(x = -3, y = avg_median_bid, label = paste("Avg Median:", round(avg_median_bid, 2))), color = "blue", vjust = -1.5,       hjust = -0.1,size=2) +
 geom_text(data = averages, aes(x = -3, y = avg_min_bid, label = paste("Avg Min:", round(avg_min_bid, 2))), color = "red", vjust = -1, hjust =           -0.1,size=2) +
  labs(title = "Window 1 Bidding Price",subtitle = "IS210 Business Process Analysis and Solutioning",y = "Bid Price") +
  scale_y_continuous(limits = c(0, 35)) +
  facet_wrap(~Year) +
  scale_color_manual(values = c("Median Bid" = "blue", "Min Bid" = "red"), name = "Bid Type")


ggsave("IS210_ANALYSIS1.png",, width = 10, height = 6)
```

## Plot the data 2 -> Bidding Price vs Instructor
```{r Bidding Price vs Instructor, echo=TRUE}
...
averages <- bidding_data_filtered %>%
  group_by(Year) %>%
  summarise(
    avg_median_bid = mean(`Median Bid`, na.rm = TRUE),
    avg_min_bid = mean(`Min Bid`, na.rm = TRUE)
  )
...
ggplot(bidding_data_filtered) +
  geom_point(mapping = aes(x = Instructor, y = `Median Bid`, color = "Median Bid"), size = 0.5) +
  geom_point(mapping = aes(x = Instructor, y = `Min Bid`, color = "Min Bid"), size = 0.5) +
  geom_text(mapping = aes(x = Instructor, y = `Median Bid`, label = `Median Bid`), vjust = -0.8, color = "blue", size = 2) +
  geom_text(mapping = aes(x = Instructor, y = `Min Bid`, label = `Min Bid`), vjust = 1.8, color = "red", size = 2) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + geom_hline(data = averages, aes(yintercept = avg_median_bid, color = "Avg Median Bid"),      linetype= "dashed") +
  geom_hline(data = averages, aes(yintercept = avg_min_bid, color = "Avg Min Bid"), linetype = "dotted") +
...
ggsave("IS210_ANALYSIS2.png",, width = 10, height = 6)
```